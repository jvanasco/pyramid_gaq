...
def gaq_trackEvent(track_dict, request=None):
    ...
    request.tmpl_context._gaq["_trackEvent"].append(
        """['_trackEvent',%s]""" % ",".join(clean)
    )
...
def gaq_setCustomVar(index, name, value, opt_scope=None, request=None):
    ...
    variable. There is a 64-byte character limit for the name and value combined.'
    ...
    if request is None:
        request = get_current_request()
    if opt_scope:
        request.tmpl_context._gaq["_setCustomVar"].append(
...
def gaq_addTrans(track_dict, request=None):
    """
    'Creates a transaction object with the given values. As with _addItem(), this
    method handles only transaction tracking and provides no additional ecommerce
    functionality. Therefore, if the transaction is a duplicate of an existing
    transaction for that session, the old transaction values are over-written with
    the new transaction values. Arguments for this method are matched by position,
    so be sure to supply all parameters, even if some of them have an empty value.'
    ...
    for i in [
        ...
        if i not in track_dict:
            track_dict[i] = ""
    request.tmpl_context._gaq["_addTrans"].append(
...
def gaq_addItem(track_dict, request=None):
    ...
    for i in ["category"]:  # fix optionals for positioning
        if i not in track_dict:
            track_dict[i] = ""
    request.tmpl_context._gaq["_addItem"].append(
...
def gaq_trackTrans(request=None):
    """
    gaq_trackTrans(request=None)- You merely have to call this to enable it.

    I decided to require this, instead of automatically calling it if a transaction
    exists, because this must be explicitly called in the ga.js API and its safer
...
def gaq_print(request=None):
    ...
    if single_push:
        single_pushes.append(
            """['_setAccount', '%s']""" % request.tmpl_context._gaq["_setAccount"]
        )
    else:
        script.append(
            """_gaq.push(['_setAccount', '%s']);"""
            % request.tmpl_context._gaq["_setAccount"]
        )
    ...
    if request.tmpl_context._gaq["_setDomainName"]:
        if single_push:
            single_pushes.append(
                """['_setDomainName', '%s']"""
                % request.tmpl_context._gaq["_setDomainName"]
            )
        else:
            script.append(
    ...
    if request.tmpl_context._gaq["_setAllowLinker"]:
        if single_push:
            single_pushes.append(
                """['_setAllowLinker', %s]"""
                % ("%s" % request.tmpl_context._gaq["_setAllowLinker"]).lower()
            )
        else:
            script.append(
